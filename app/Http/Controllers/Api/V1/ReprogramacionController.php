...
use App\Http\Requests\V1\BulkDeleteReprogramacionRequest;
...
class ReprogramacionController extends Controller
{
    ...
    public function destroyBulk(BulkDeleteReprogramacionRequest $request)
    {
        ...
        return \DB::transaction(function () use ($ids) {
            ...
            $deleted = \App\Models\Reprogramacion::whereIn('id', $ids)->delete();

            return response()->json([
                'ok'      => true,
                'message' => 'Reprogramaciones eliminados correctamente.',
                'counts'  => [
                    'requested' => count($ids),
                    'deleted'   => (int) $deleted,
                ],
