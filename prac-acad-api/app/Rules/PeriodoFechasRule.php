...
class PeriodoFechasRule implements RuleContract
{
    ...
    public function passes($attribute, $value): bool
    {
        ...
        try {
            $fecha = \Carbon\Carbon::createFromFormat('Y-m-d', $value)->startOfDay();
        } catch (\Exception $e) {
            $this->msg = 'La fecha debe tener formato Y-m-d.';
            return false;
        }

        $inicio = \Carbon\Carbon::create($anio, $sem === 1 ? 1 : 7, 1)->startOfDay();
        $fin    = $sem === 1
            ? \Carbon\Carbon::create($anio, 6, 30)->endOfDay()
            : \Carbon\Carbon::create($anio, 12, 31)->endOfDay();

        if ($fecha->lt($inicio) || $fecha->gt($fin)) {
            // Mensaje específico que pides:
